...
- `request_capture()` は要求を受信した時点より前の client scene を snapshot し、その scene の build 完了後に、要求直前の `set_camera()` 指定（なければ要求時点の viewer カメラ）でレンダリングして保存します。画像の保存が完了するまで、viewer のカメラ操作と後続 client scene の表示反映は停止します。保存中に届いた後続 scene は保存完了後に反映されます。`request_capture(false)` は accumulation 完了を待たず、scene build 完了後の最初のサンプルを保存します。\
