...
bool wide_to_utf8(const wchar_t* text, std::string* out_utf8) {
    ...
    out_utf8->assign(static_cast<std::size_t>(size - 1), '\0');
    WideCharToMultiByte(CP_UTF8, 0, text, -1, out_utf8->data(), size, nullptr, nullptr);
    return true;
#else
    out_utf8->clear();
    while (*text != L'\0') {
...
bool write_png_rgba8(const wchar_t* path, const std::uint8_t* rgba, int width, int height, int stride) {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        std::uint8_t* dst_row = tightly_packed.data() + static_cast<std::size_t>(y) * static_cast<std::size_t>(width) * 4u;
